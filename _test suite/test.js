...
function test_era() {
	...
	error_count += CeL.test('紀年/曆數', [
		...
		[['庚辰年庚辰月庚辰日 2', '清德宗光緒六年三月十三日'.to_Date('era').format({parser:'CE',format:'%歲次年%月干支月%日干支日 %w',locale:'cmn-Hant-TW'})], '統一時間標準→各特殊紀年（西→中）：查詢某時間點（時刻）的日期資訊，如月干支等。'],
		[['庚辰年庚辰月庚辰日庚辰時 二', '一八八〇年庚辰月庚辰日7時'.to_Date({parser:'era',range:'中國'}).format({parser:'CE',format:'%歲次年%月干支月%日干支日%時干支時 %a',locale:'cmn-Hant-TW'})], '各特殊紀年→統一時間標準（中→西）：查詢某農曆+紀年/君主(帝王)對應的標準時間(如UTC+8)。'],
		[['庚辰年庚辰月庚辰日庚辰時 星期二', '一八八〇年庚辰月庚辰日庚辰時'.to_Date({parser:'era',range:'中國'}).format({parser:'CE',format:'%歲次年%月干支月%日干支日%時干支時 %A',locale:'cmn-Hant-TW'})], '各特殊紀年→統一時間標準（中→西）：查詢某農曆+紀年/君主(帝王)對應的時辰。'],
