...
function test_native() {
	...
	all_error_count += CeL.test('between(): character.length > 1', [
		...
		[['0123456789123456789'.between(null, '012'), '']],
	...
	all_error_count += CeL.test('data.native misc', function (assert) {
		...
		assert(['', [].sort().unique_sorted().join('|')], '.unique_sorted() #0');
		assert(['21', [21].sort().unique_sorted().join('|')], '.unique_sorted() #1');
		assert(['3|4|6|7|8|9|56|79|a34|a6', '3,4,6,7,7,8,9,9,9,56,56,79,79,a34,a34,a6'.split(',').unique_sorted().join('|')], '.unique_sorted() #2');
		assert(['2|3|4|5', [3, 4, 2, 2, 5, 4, 3].sort().unique_sorted().join('|')], '.unique_sorted() #3');
...
function test_wiki() {
	...
	all_error_count += CeL.test('wiki: CeL.wiki.parser()', function (assert) {
		...
		wikitext = 'https://[2001:db8:85a3:8d3:1319:8a2e:370:7348]:443/'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: plain url #7');
		assert(['url', parsed.type], 'wiki.parse: plain url #7-1');
		wikitext = '{{text|urn:0{{text| |urn:1}}}}'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: plain url #8');
		assert(['url', parsed.parameters[1][0].type], 'wiki.parse: plain url #8-1');
		assert(['url', parsed.parameters[1][1].parameters[2].type], 'wiki.parse: plain url #8-1');
		...
		assert([wikitext, parsed.toString()], 'wiki.parse: plain url #11');
		assert(['plain', parsed.type], 'wiki.parse: plain url #11-1');
		assert(['url', parsed[0].type], 'wiki.parse: plain url #11-2');
		...
		assert(["", parsed[1]], 'wiki.parse: external link #4-1');
		assert(["''disply'' text", parsed[2].toString()], 'wiki.parse: external link #4-2');
		...
		wikitext = "[//example.com t]"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: external link [//...] #1');
		assert(["external_link", parsed.type], 'wiki.parse: external link [//...] #2');
		wikitext = "[[http://example.com]]"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: external link #9');
		...
		assert(['[', parsed[0]], 'wiki.parse: external link #10-2');
		assert(['external_link', parsed[1].type], 'wiki.parse: external link #10-3');
		assert(['[http://example.com foo bar]', parsed[1].toString()], 'wiki.parse: external link #10-4');
		wikitext = "[[mailto:u@example.org foo bar]]"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: external link: [[url]] #2');
		assert(['plain', parsed.type], 'wiki.parse: external link: [[url]] #2-1');
		assert(['[', parsed[0]], 'wiki.parse: external link: [[url]] #2-2');
		assert(['external_link', parsed[1].type], 'wiki.parse: external link: [[url]] #2-3');
		assert(['[mailto:u@example.org foo bar]', parsed[1].toString()], 'wiki.parse: external link: [[url]] #2-4');
		...
		assert(['url', parsed[0].type], 'wiki.parse: external link #12-2');
		assert(['http://example.com/foo{{!}}bar', parsed[0].toString()], 'wiki.parse: external link #12-3');
		assert(['', parsed[1]], 'wiki.parse: external link #12-4');
		assert(['italic', parsed[2].type], 'wiki.parse: external link #12-5');
		assert(["''foo bar''", parsed[2].toString()], 'wiki.parse: external link #12-6');
		wikitext = "[http://example.com/foo<!---->bar{{text|}} t]"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: external link #13');
		assert(['external_link', parsed.type], 'wiki.parse: external link #13-1');
		assert(['url', parsed[0].type], 'wiki.parse: external link #13-2');
		assert(['http://example.com/foo<!---->bar', parsed[0].toString()], 'wiki.parse: external link #13-3');
		...
		assert(['link', parsed[1].type], 'parse link type');
		parsed.each('link', function (token, index, parent) { return '{{t}}'; }, true);
		assert(['a{{t}}b', parsed.toString()], 'Modify link');
		...
		wikitext = '[[未來日記-ANOTHER:WORLD-]]'; parsed = CeL.wiki.parse(wikitext);
		...
		wikitext = '[[WP:未來日記-ANOTHER:WORLD-]]'; parsed = CeL.wiki.parse(wikitext);
		...
		wikitext = '{{|t}}'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed], 'wiki.parse.transclusion #26-1');
		wikitext = '{{ |t}}'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed], 'wiki.parse.transclusion #27-1');
		wikitext = '{{ 　 |t}}'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed], 'wiki.parse.transclusion #28-1');
		wikitext = '{{ <!----> |t}}'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.transclusion #29');
		assert(['plain', parsed.type], 'wiki.parse.transclusion #29-1');
		wikitext = '{{t\n <!-- -->}}'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.transclusion #30');
		assert(['transclusion', parsed.type], 'wiki.parse.transclusion #30-1');
		wikitext = '{{\n <!-- --> t}}'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.transclusion #31');
		assert(['transclusion', parsed.type], 'wiki.parse.transclusion #31-1');
		wikitext = '{{\n <!-- --> t n \n<!-- -->}}'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.transclusion #32');
		assert(['transclusion', parsed.type], 'wiki.parse.transclusion #32-1');
		wikitext = '{{\n <!-- --> t\nn \n<!-- -->}}'; parsed = CeL.wiki.parse(wikitext);
		...
		assert(['plain', parsed.type], 'wiki.parse.transclusion #46-1');
		assert(['tag', parsed[1].type], 'wiki.parse.transclusion #46-2');

		wikitext = '{{tl\n \n <!-- -->\n |a}}'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.transclusion #47');
		...
		assert([wikitext, parsed], 'wiki.parse: invalid wikilink');
		wikitext = '[[<!-- -->_|a]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: invalid wikilink');
		assert(['plain', parsed.type], 'wiki.parse: invalid wikilink');
		wikitext = '[[<s>s</s>]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: invalid wikilink');
		assert(['plain', parsed.type], 'wiki.parse: invalid wikilink');
		wikitext = '[[t<s>s</s>]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: invalid wikilink');
		assert(['plain', parsed.type], 'wiki.parse: invalid wikilink');
		...
		assert(['plain', parsed.type], 'wiki.parse: invalid wikilink');
		wikitext = "[[''i'']]"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: valid wikilink');
		assert(['link', parsed.type], 'wiki.parse: valid wikilink');
		wikitext = "[['''b''']]"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: valid wikilink');
		assert(['link', parsed.type], 'wiki.parse: valid wikilink');
		wikitext = "[[==s==]]"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: valid wikilink');
		assert(['link', parsed.type], 'wiki.parse: valid wikilink');
		wikitext = '[[ ftp://example.org]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link invalid wikilink');
		assert(['plain', parsed.type], 'wiki.parse.link invalid wikilink');
		assert(['[[ ', parsed[0]], 'wiki.parse.link invalid wikilink');
		assert(['url', parsed[1].type], 'wiki.parse.link invalid wikilink');
		assert(['ftp://example.org', parsed[1].toString()], 'wiki.parse.link invalid wikilink');
		wikitext = '[[ https://example.org |t]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link invalid wikilink');
		assert(['plain', parsed.type], 'wiki.parse.link invalid wikilink');
		assert(['[[ ', parsed[0]], 'wiki.parse.link invalid wikilink');
		assert(['url', parsed[1].type], 'wiki.parse.link invalid wikilink');
		assert(['https://example.org', parsed[1].toString()], 'wiki.parse.link invalid wikilink');
		wikitext = '[[ mailto:u@example.org]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link invalid wikilink');
		assert(['plain', parsed.type], 'wiki.parse.link invalid wikilink');
		assert(['[[ ', parsed[0]], 'wiki.parse.link invalid wikilink');
		assert(['url', parsed[1].type], 'wiki.parse.link invalid wikilink');
		assert(['mailto:u@example.org', parsed[1].toString()], 'wiki.parse.link invalid wikilink');
		wikitext = '[[ nonprotocol://example.org]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link valid wikilink');
		assert(['link', parsed.type], 'wiki.parse.link valid wikilink');
		wikitext = '[[ nonprotocol://example.org|https://example.org]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link valid wikilink');
		assert(['link', parsed.type], 'wiki.parse.link valid wikilink');
		wikitext = '[[ //example.org]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed], 'wiki.parse.link invalid wikilink');
		wikitext = '[[ ://example.org]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link valid wikilink');
		assert(['link', parsed.type], 'wiki.parse.link valid wikilink');
		wikitext = '[[ a//example.org]]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link valid wikilink');
		assert(['link', parsed.type], 'wiki.parse.link valid wikilink');
		wikitext = '[ https://example.org |t]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link invalid external_link');
		assert(['plain', parsed.type], 'wiki.parse.link invalid external_link');
		assert(['[ ', parsed[0]], 'wiki.parse.link invalid external_link');
		assert(['url', parsed[1].type], 'wiki.parse.link invalid external_link');
		assert(['https://example.org', parsed[1].toString()], 'wiki.parse.link invalid external_link');
		wikitext = '[https://example.org |t]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link valid external_link');
		assert(['external_link', parsed.type], 'wiki.parse.link valid external_link');
		assert(['|t', parsed[2]], 'wiki.parse.link invalid external_link');
		wikitext = '[//<ref></ref>]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link invalid external_link');
		assert(['[//', parsed[0]], 'wiki.parse.link invalid external_link');
		wikitext = '[<!---->//<ref></ref>]'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse.link invalid external_link');
		...
		wikitext = "'''b''bi'''i''"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()]);
		assert(['plain', parsed.type], 'wiki.parse: bold-italic');
		assert(["bold", parsed[0].type], 'wiki.parse: bold-italic');
		assert(["'''b''bi'''", parsed[0].toString()], 'wiki.parse: bold-italic');
		assert(["italic", parsed[1].type], 'wiki.parse: bold-italic');
		assert(["i''", parsed[1].toString()], 'wiki.parse: bold-italic');
		wikitext = "''i'''bi''b'''"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()]);
		assert(['plain', parsed.type], 'wiki.parse: bold-italic');
		assert(["italic", parsed[0].type], 'wiki.parse: bold-italic');
		assert(["''i'''bi''", parsed[0].toString()], 'wiki.parse: bold-italic');
		assert(["bold", parsed[1].type], 'wiki.parse: bold-italic');
		assert(["b'''", parsed[1].toString()], 'wiki.parse: bold-italic');
		wikitext = "'''i''t'''b''bi'''i''t"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()]);
		assert(['plain', parsed.type], 'wiki.parse: bold-italic');
		assert(["'", parsed[0]], 'wiki.parse: bold-italic');
		assert(["italic", parsed[1].type], 'wiki.parse: bold-italic');
		assert(["''i''", parsed[1].toString()], 'wiki.parse: bold-italic');
		assert(["'''b''bi'''", parsed[3].toString()], 'wiki.parse: bold-italic');
		assert(["italic", parsed[4].type], 'wiki.parse: bold-italic');
		wikitext = "'''b''bi'''i''t'''b''bi'''i''t"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()]);
		assert(['plain', parsed.type], 'wiki.parse: bold-italic');
		assert(["bold", parsed[0].type], 'wiki.parse: bold-italic');
		assert(["'''b''bi'''", parsed[0].toString()], 'wiki.parse: bold-italic');
		assert(["italic", parsed[1].type], 'wiki.parse: bold-italic');
		assert(["i''", parsed[1].toString()], 'wiki.parse: bold-italic');
		wikitext = "''i'''bi''''i"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()]);
		assert(["italic", parsed.type], 'wiki.parse: bold-italic');
		assert([wikitext, parsed.toString()], 'wiki.parse: bold-italic');
		...
		wikitext = '<section begin=<section begin=s1 />\ntext\n<section end=s1 />\n\n{{#section:Wikipedia:Sandbox|s1}}\n'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: HTML tag section #1');
		...
		assert([wikitext, CeL.wiki.parse.replace_parameter(token, CeL.wiki.parse.replace_parameter.KEY_template_name, '  t ') === 1 && token.toString()], 'wiki.parse.replace_parameter: #18-2 Only change template name');
		var parameters = { a: 1 };
		parameters[CeL.wiki.parse.replace_parameter.KEY_template_name] = 't';
		assert([wikitext, CeL.wiki.parse.replace_parameter(token, parameters, { value_only: true, no_value_space: true }) === 0 && token.toString()], 'wiki.parse.replace_parameter: #18-3');
		parameters.v = 3;
		assert([wikitext, CeL.wiki.parse.replace_parameter(token, parameters, { value_only: true, no_value_space: true }) === 0 && token.toString()], 'wiki.parse.replace_parameter: #18-4');
		parameters.a = 2;
		...
		assert([wikitext, parsed], 'wiki.section_link #7-1 正常情況下 "[[ ]]" 不會被 parse 成 link。');
		wikitext = "[[ #]]"; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.section_link #8');
		assert([wikitext, parsed], 'wiki.section_link #8-1 正常情況下 "[[ #]]" 不會被 parse 成 link。');
		wikitext = "[[#]]"; parsed = CeL.wiki.parse(wikitext);
		...
		assert([' :: ', parsed/* ← type: 'table' */.indentation], 'wiki.parse: table #8-1');
		wikitext = '{|class="wikitable"\n|-\n|[[T|t]]\n|}'; parsed = CeL.wiki.parse(wikitext);
		assert([wikitext, parsed.toString()], 'wiki.parse: table #9');
		assert(['link', parsed/* ← type: 'table' */[1]/* ← table_row */[0]/* ← table_cell */[0].type], 'wiki.parse: table #9-1');
		...
		wikitext = '{{t|_1|_2|s=1|_3|_4}}'; parsed = CeL.wiki.parse(wikitext);
		parsed.splice(2, 3, 'y=t|4', 'ddd|q=1');
		parsed.toString();
...
function test_era() {
	...
	all_error_count += CeL.test('紀年/曆數', [
		[CeL.era('前1年8月15日').共存紀年.join().includes('漢哀帝元壽2年6月26日'), '前1年8月15日 共存紀年 漢哀帝元壽2年6月26日'],
		[['西漢哀帝元壽2年6月26日', CeL.era('前1年8月15日').format({ parser: 'CE', format: '%紀年名%年年%月月%日日', locale: 'cmn-Hant-TW', as_UTC_time: true })], '前1年8月15日 朝代紀年日期 漢哀帝元壽2年6月26日'],
	...
	all_error_count += CeL.test('.共存紀年 test: 可能需要因添加紀年而改變。', [
		...
		[CeL.era('延熙1年5月2日').共存紀年.join('|').covers('弥生時代神功皇后38年5月3日|高句麗東川王12年6月3日|新羅助賁尼師今9年6月3日|吳大帝嘉禾7年5月3日|百濟古尒王5年6月3日|魏明帝景初2年6月3日|魏燕王紹漢2年6月3日'), '各特殊紀年→統一時間標準（中→西）：查詢某朝代/君主(帝王)所有的共存紀年與資訊。#1'],
