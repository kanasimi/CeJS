 ...
 * @fileoverview
 * 本檔案包含了編碼，例如自動偵測地區語系/文化設定編碼的 functions。言語判定ライブラリ。
 * @since	
 */
...
require : 'data.|data.native.|application.OS.Windows.new_COM|application.locale.language_tag|application.OS.Windows.file.open_file|application.OS.Windows.file.is_file|application.OS.Windows.file.AdoEnums|application.OS.Windows.file.translate_ADO_Stream_binary_data',
 ...
 * 偵測檔案編碼。現可偵測中日韓東亞語言。
 * @param {String}file_path	檔案path
 * @param {Boolean}[is_HTML]	為 HTML 檔案
 * @param [open_function]	open_function(file_path, length, encoding)
 * @returns
 * @see
 * <a href="http://zh.wikipedia.org/wiki/%E5%AD%97%E7%AC%A6%E9%9B%86%E6%8E%A2%E6%B5%8B" accessdate="2012/3/17 20:7" title="Character encoding detection, charset detection, or code page detection is the process of heuristically guessing the character encoding of a series of bytes that represent text.">字符集探测</a>,
 * <a href="http://svn.perian.org/trunk/UniversalDetector/universalchardet/doc/UniversalCharsetDetection.doc" accessdate="2012/2/25 9:26">A composite approach to language/encoding detection</a>,
 * <a href="http://xygszsh.blog.163.com/blog/static/1922151720102230136335/" accessdate="2012/2/25 9:13" title="一种语言/编码检测的复合方法（转） - 少華的日志 - 网易博客">一种语言/编码检测的复合方法</a>,
 * <a href="http://blog.csdn.net/marising/article/details/1845038" accessdate="2012/2/25 9:14" title="一种语言/编码检测的复合方法 - 李海波 -- 做影响一亿人的产品 - 博客频道 - CSDN.NET">一种语言/编码检测的复合方法</a>,
 * <a href="http://cs229.stanford.edu/proj2007/KimPark-AutomaticDetectionOfCharacterEncodingAndLanguages.pdf" accessdate="2012/2/25 9:14" title="Automatic Detection of Character Encoding and Language">Automatic Detection of Character Encoding and Language</a>,
 */
function guess_encoding(file_path, is_HTML, options) {
	...
	//if(typeof _.get_HTML_encoding!='function')is_HTML=false;
	if (!is_file(file_path)) {
		library_namespace.debug('Treat [' + file_path + '] as string.');
		return file_path.length < 1024 ? guess_encoding.unknown_encoding : (t = guess_text_language(file_path, undefined, options)) ? t
	...
	if (isNaN(reading_length) || (reading_length |= 0) < 0 || reading_length > guess_encoding.max_length_to_test)
		// default
		reading_length = is_HTML ? guess_encoding.min_length_of_HTML
			: guess_encoding.min_length_to_test;

	t = ADO_Stream.ReadText(3);//Read(3);//
	//library_namespace.debug(file_path + ': [' + t.slice(0, 3).split('').join(',') + ']..', 2);

	if (typeof is_HTML === 'undefined')
		is_HTML = /\.[xs]?html?$/i.test(file_path);

	var question_mark_count;
	//if(typeof t!='string')return guess_encoding.unknown_encoding;//t=''+t;	//	此時type通常是unknown，不能用+=
	...
	if (t.slice(0, 2) === '\xFF\xFE')
		//	Unicode big-endian
		code = 'unicodeFFFE';//'UTF-16BE';//'unicodeFFFE';
		...
		var l, codes = {}, reg = new RegExp(), stream = open_file(file_path, 'binary');
		codes[binary_charset] = '[\\x09\\x0a\\x0d\\x20-\\x7e]';
		//codes['Big5']=codes[binary_charset]+'|[\\xa4-\\xc6\\xc9-\\xf9][\\x40-\\xfe]';	//	http://www.cns11643.gov.tw/web/word/big5/index.html
		//codes['Shift_JIS']=codes[binary_charset]+'|[\\x81-\\x9f\\xe0-\\xef\\xfa-\\xfc][\\x40-\\x7e\\x80-\\xfc]|[\\xa1-\\xdf]';	//	http://hp.vector.co.jp/authors/VA013241/misc/shiftjis.html
		//codes['EUC-JP']=codes[binary_charset]+'|\\x8f[\\xa1-\\xfe][\\xa1-\\xfe]|[\\xa1-\\xfe][\\xa1-\\xfe]|\\x8e[\\xa1-\\xdf]';
		codes['UTF-8'] = codes[binary_charset]
			+ '|[\\xc0-\\xdf][\\x80-\\xbf]|[\\xe0-\\xef][\\x80-\\xbf]{2}|[\\xf0-\\xf7][\\x80-\\xbf]{3}'
			+ '|[\\xf8-\\xfb][\\x80-\\xbf]{4}|[\\xfc-\\xfd][\\x80-\\xbf]{5}';
		//codes['GB 2312']=codes[binary_charset]+	//	GBK
		...
		if (!is_HTML
				...
				//&& /^(?:[^<>]+|<\/?[\s\S]+?>)*$/.test(code)
				// 以下 OK.
				&& !/[<>]/.test(code.replace(/<\/?[a-z][^<>]*>/gi, '').replace(/<(?:[a-z][^<>]*)?$/, ''))
				) {
			library_namespace.debug('Treat [' + file_path + '] as HTML.', 1, 'guess_encoding');
			is_HTML = true;
			// reading_length === 0: 已 read all.
			if (reading_length && reading_length < guess_encoding.min_length_of_HTML) {
				library_namespace.debug('原先取樣文本長度: ' + reading_length + ' 過短，重新讀取長度: ' + guess_encoding.min_length_of_HTML + '。', 2);
				reading_length = guess_encoding.min_length_of_HTML;
				t = translate_ADO_Stream_binary_data(
						t, reading_length);
			} else
				t = code;
		} else
			t = code;
		code = undefined;
		library_namespace.debug('取樣文本 (' + t.length + ') [<span style="color:#b99;">' + t.replace(/</g, '&lt;') + '</span>]', 2, 'guess_encoding');
		//	另可使用 .split(',').length - 1
		question_mark_count = t.count_of('?');
		library_namespace.debug("question mark '?' count = " + question_mark_count, 2, 'guess_encoding');
	...
	library_namespace.debug('coding: [' + code + '] in phase 1.', 2, 'guess_encoding');
	...
	if (is_HTML) {
		...
		if (t = _.get_HTML_encoding(reading_length ? ADO_Stream.ReadText(reading_length)
			//ADO_Stream.ReadText(adReadAll)
			: ADO_Stream.ReadText()))
			code = t;

		library_namespace.debug('coding: [' + code + '] in phase 2.', 2, 'guess_encoding');
	}
	...
	if (!code) {
		var enc, enc_tmp, lang_code = guess_encoding.test_group, c,
		test_group = {}, EMPTY_TEST_GROUP = 0,
		most_probable_code_changed;
		//	初始化 test_group.
		//test_group['EUC-KR']={'EUC-KR':1, 'EUC-JP':1};
		//test_group['EUC-JP']={'EUC-KR':1, 'EUC-JP':1};
		...
		for (enc in guess_encoding.mapping) {
			...
			//ADO_Stream.ReadText(adReadAll)
			: ADO_Stream.ReadText();
			//library_namespace.debug(enc + '(' + ADO_Stream.Size + '):<br />' + t.slice(0, 200), 1);

			if (enc === 'EUC-JP') {
				//	半角・全角形[ｦ-ﾝ] 可能在以 'EUC-JP' 讀取 'EUC-KR' 或 'GB 2312' 時大量出現而造成誤判。加以日本語文件本身也少僅用這些字母，因此割愛。
				t = t.replace(/[\uFF66-\uFF9D]+/g, '');

				i = t.replace(/[\t\x20-\x7f]+/g, '')
				//	Windows 下， EUC-JP 對無法編碼者會改成 '・' 而非 '�'。
				if (library_namespace.is_debug(2))
					library_namespace.debug('[・] count: ' + i.count_of('・') + ' / ' + i.length + '=' + (i.count_of('・') / i.length) + ' @ ' + enc, 1, 'guess_encoding');
				// 計算 '・' 佔非 ASCII 之比例。
			...
			//	可惜 ADO_Stream.ReadText 僅會把無法編碼者改成 '?'，而不會改成 replacement character '�'。
			//	TODO: '�' 的比例過大時(e.g., ratio>.0001 && count>1)則 pass.
			if ((i = t.indexOf('�')) === -1) {
				unknown_character_count = t.count_of('?') - question_mark_count;
				if (library_namespace.is_debug(2)) {
					library_namespace.debug("question mark '?' count of [" + enc + "] = " + unknown_character_count, 2, 'guess_encoding');
				}
			...
			if (unknown_character_count / t.length > .001) {
				library_namespace.debug('看來似乎不是 encoding [' + enc + ']! Unknown characters: ' + unknown_character_count + ' / ' + t.length + ' = ' + (unknown_character_count / t.length), 2, 'guess_encoding');
				continue;
			}


			lang_code = guess_encoding.mapping[enc];
			library_namespace.debug('Test charset [' + enc + ' (' + lang_code + ')' + '] decoded as (' + t.length + ') [<span style="color:#aaa;">' + t.replace(/</g, '&lt;') + '</span>]', 2, 'guess_encoding');
			c = guess_text_language(t, lang_code, Object.assign({
				return_ratio : true
			}, options));
			library_namespace.debug(function(){return 'Wish '+ lang_code + ' and get ' + c;}, 3, 'guess_encoding');
			most_probable_code_changed = false;
			for (i in c)
				if (c[i] > highest_ratio) {
					library_namespace.debug('Most probable code now: [' + enc + '].[' + i + '] = ' + c[i], 2, 'guess_encoding');
			...
			if(most_probable_code_changed || test_group[enc] === EMPTY_TEST_GROUP) {
				if(most_probable_code_changed)
					library_namespace.debug('Test ' + enc + ': [' + lang_code + '] and get [' + most_probable_code + '].', 2, 'guess_encoding');
				...
				if (highest_ratio >= guess_encoding.default_select_boundary) {
					if(!(enc in test_group)) {
						...
						for (i in test_group[enc_tmp]) {
							library_namespace.debug('由於 ' + enc + ' @ [' + test_group[enc_tmp][i] + ']　編碼類似，有時會產生誤判，因此持續作測試以找出最可能之編碼。', 2, 'guess_encoding');
							break;
						}
					}
				}
...
//	@see	<a href="http://en.wikipedia.org/wiki/IETF_language_tag" accessdate="2012/8/22 15:25">IETF language tag</a> (script code)

//	TODO: EUC-TW, ISO2022-XX，和HZ。
//	TODO: 提供一種通用模式來處理單字節編碼 - 俄語編碼(KOI8-R, ISO8859-5, window1251, Mac-cyrillic, ibm866, ibm855)
//	TODO: parse <a href="http://tools.ietf.org/html/rfc5646">IANA language tag</a>: cmn-Hant-TW, cmn-Hans-CN
//		@see	http://www.cnblogs.com/sink_cup/archive/2010/07/01/language_subtag_registry.html
guess_encoding.mapping = {
		//	<a href="http://zh.wikipedia.org/wiki/EUC" accessdate="2011/12/28 15:21" title="Extended Unix Code (EUC)">EUC</a>
		...
		//	將 GB 2312 排在 Big5 前面是因為 GB 2312 常用字在 Big5 中常常是\u8000之後的常用字，Big5 常用字卻常常是 GB 2312 中奇怪字碼與罕用字。
		'GB 2312' : {
		...
		//	<a href="http://zh.wikipedia.org/wiki/%E9%98%BF%E6%8B%89%E4%BC%AF%E5%AD%97%E6%AF%8D" accessdate="2012/3/17 20:26">阿拉伯字母</a>,
		//	<a href="http://baheyeldin.com/arabization/history-of-arabic-on-computers.html" accessdate="2012/3/17 21:2">Arabic on the Internet: History of Arabic on Computers | The Baheyeldin Dynasty</a>.
		//	ar-SA — Arabic
		...
		//	<a href="http://zh.wikipedia.org/wiki/%E4%BF%84%E8%AF%AD%E5%AD%97%E6%AF%8D" accessdate="2012/3/17 20:27">俄語字母</a>,
		//	<a href="http://rlandmann.fedorapeople.org/pug/appe-Users_Guide-Language_codes.html" accessdate="2012/3/17 23:19">Appendix D. Language codes</a>
		//	ru-RU — Russian
		'Windows-1251' : 'ru-RU',
...
is_HTML_file = function(text) {
	...
	var HTML_pattern = /^(([\s\n]*<[\w?!][^>]*>)*?)[\s\n]*<html([\s\n][^>]*)?>/i;
	//library_namespace.debug('is_HTML_file: text is ' + (HTML_pattern.test(text) ? '' : '<em>NOT</em> ') + 'HTML document.');
 ...
 * detect language.<br />
 * TODO:
 * <a href="http://zh.wikipedia.org/wiki/%E5%A4%A9%E5%9F%8E%E6%96%87" accessdate="2012/3/17 20:27">天城文</a>,
 * <a href="http://zh.wikipedia.org/wiki/%E5%AD%9F%E5%8A%A0%E6%8B%89%E6%96%87" accessdate="2012/3/17 20:27">孟加拉文</a>
 * 
 * @param {String}text
 *            文本或字串
 ...
 *        2011/12/28 22:28:35	refactoring: 重構以判別 EUC-KR。
 *        2012/3/17 17:16:32	兩段式判別：降低不常用字之比重。
 *        2012/3/17 23:20:20	Adding Arabic, Russian.
...
function guess_text_language(text, language_to_test, options) {
	...
	library_namespace.debug(text.length + ' characters after remove_support_contents(): [<span style="color:#9b9;">' + text.replace(/</g, '&lt;') + '</span>]', 2);
	...
	seldom_character_count = {},
	return_ratio = options.return_ratio, test_all = options.test_all,
	signature_RegExp = guess_text_language.get_signature_RegExp(),
	remove_lang = function(lang, add_tag) {
		var filter = lang;
		...
		if (old_length && signature_RegExp[filter]) {
			if (library_namespace.is_debug(2)) {
				if (unrecognized = text.match(signature_RegExp[filter])) {
					...
					library_namespace.debug('character of ' + filter + ': (' + unrecognized.length + ') [' + unrecognized.slice(0, 80) + ']' + (unrecognized.length > 80 ? '..' : ''), 2, 'guess_text_language');
				}
			}
			...
			if(add_tag || old_length !== unrecognized) {
				if(add_tag !== seldom_character_count)
					...
					seldom_character_count[lang] = old_length - unrecognized;
	...
	if (library_namespace.is_debug(2)) {
		for (i in character_count) {
			if (character_count[i])
				library_namespace.debug('count ' + i + ': ' + character_count[i], 2, 'guess_text_language');
		}
		if(unrecognized)
			library_namespace.debug(unrecognized + ' unknown characters: [' + text.slice(0, 300) + ']');
	}
	...
	if ((character_count['en-US'] + (seldom_character_count['en-US'] || 0))
			...
			&& character_count['en-US'] / (character_count.all - unrecognized) > .5)
		return 'en-US';

	featured = character_count.all - character_count['en-US'] - (seldom_character_count['en-US'] || 0) - character_count['x-general'];
	library_namespace.debug('unrecognized ' + unrecognized + ' / featured ' + featured + ' = ' + +((unrecognized / featured).toFixed(3)) + ',  boundary ' + guess_text_language.unrecognized_boundary, 2);
	if (
			// 判斷已知的同時，未知字碼需要極少…不過這在遇上符號時可能會出現錯誤。因此最好將符號排除。
			unrecognized / featured > guess_text_language.unrecognized_boundary) {
		if (library_namespace.is_debug(2)) {
			library_namespace.debug('unrecognized > boundary: total ' + text.length + ' unknown characters.', 2);
			for ( var i = 0, l = text.length; i < l; i++) {
				library_namespace.debug('[<em class="Unicode">' + text.charAt(i) + '</em>]: U+' + text.charCodeAt(i).toString(16).toUpperCase() + ' (' + text.charCodeAt(i) + '<span class="down">10</span>)', 3);
			}
		}
		//	若 ratio === 1 但指定 language_to_test{}，可能造成回傳與原先結構不同之 language_to_test{}!
		return return_ratio ? {} : undefined;
	}

	var count, seldom_count, denominator, ratio,
	//	best confidence
	most_probable_code, highest_ratio = 0,
	language, recognized_featured_characters = featured - unrecognized - character_count['x-CJK'];
	library_namespace.debug('recognized featured characters: ' + recognized_featured_characters, 2);

	if (options.contains_JP && (i = character_count['ja-JP'])) {
		library_namespace.debug('Add JP count to TW, CN.', 2);
		character_count['cmn-Hant-TW'] = (character_count['cmn-Hant-TW'] || 0) + i;
		character_count['cmn-Hans-CN'] = (character_count['cmn-Hans-CN'] || 0) + i;
	}


	if (i = character_count['x-CJK']) {
		//	由於 CJK 於各語言各有不同比例，因此加點比重至此。各比例為依據經驗而得之 magic number。
		character_count['cmn-Hant-TW'] = (character_count['cmn-Hant-TW'] || 0) + i * .2;
		character_count['cmn-Hans-CN'] = (character_count['cmn-Hans-CN'] || 0) + i * .15;
	...
	for (i in language_to_test) {
		...
		denominator = recognized_featured_characters + character_count['x-CJK'] * .2
		...
		else if (options.contains_JP && (i === 'cmn-Hant-TW' || i === 'cmn-Hans-CN'))
			count += character_count['x-CJK'], denominator += character_count['x-CJK'];

		ratio = (count + seldom_count * guess_text_language.seldom_weight) / denominator;
		library_namespace.debug('test language [' + i + ']: ' + count + ' + ' + seldom_count + ' / (all featured characters ' + recognized_featured_characters + (recognized_featured_characters === denominator ? '' : ' → ' + denominator) + ') ≈ ' + +(ratio.toFixed(3)) + ' (boundary: ' + (language_to_test[i] || guess_text_language.default_boundary) + ')', 2);
		if (return_ratio) {
			// 設定好 ratio
			language_to_test[i] = ratio;
		}

		if (//count > (most_probable_code ? character_count[most_probable_code] : 0) &&
				ratio > Math.max(guess_text_language.default_boundary, highest_ratio)) {
			if(ratio >= (test_all ? 1 : language_to_test[i] || guess_text_language.default_select_boundary)) {
				library_namespace.debug('return the most probable code [' + i + ']: ' + ratio + '.', 2, 'guess_text_language');
				return return_ratio ? language_to_test : i;
			}
			library_namespace.debug('Most probable code now: [' + i + '] = ' + ratio, 1, 'guess_text_language');
	...
	//	經過廝殺戰的才當作有其價值。
	if(!test_all)
		library_namespace.debug('沒有所佔比例超過門檻，且可以準確判斷的 code。 ', 2, 'guess_text_language');
	library_namespace.debug('the most probable code [' + most_probable_code + ']: ' + highest_ratio, 2, 'guess_text_language');
...
//	某種語言之字元數大於此界線閥值，即視為此種語言。依據經驗而得之 magic number。應 > Math.max(.5, guess_text_language.default_boundary)。若低於此，則進入廝殺戰。
...
guess_text_language.remove_support_contents = function(text) {
	...
	if (_.is_HTML_file(text)) {
		if (library_namespace.is_debug(3)) {
			library_namespace.debug('<em>1.</em> ' + text.length + ' characters: [' + text.replace(/</g, '&lt;') + ']');
			library_namespace.debug('<em>2.</em> ' + text.length + ' characters: [' + text.replace(/<!--[\s\S]*?-->/g, '').replace(/</g, '&lt;') + ']');
			library_namespace.debug('<em>3.</em> ' + text.length + ' characters: [' + text.replace(/<!--[\s\S]*?-->/g, '').replace(/<script([\s\n][^>]*)?>[\s\S]*?<\/script>/ig, '').replace(/</g, '&lt;') + ']');
		}
	...
	//	<a href="http://ja.wikipedia.org/wiki/Unicode#.E4.B8.80.E8.A6.A7" accessdate="2012/3/17 14:17">Unicode一覧</a>
	return text.replace(
			/[\s\n\d\0-\32<>()\[\]\-\\\/\|,.*'"·©°»\u00A0\u0250-\u036F\u2000-\u27FF\u3300-\u33FF\uFF01-\uFF65]+/g, '');
};
...
//nl-NL, fr-FR, it-IT, de-DE, es-ES, tr-TR, pl-PL, cs-CZ, bg-BG, pt-PT, sl-SI, sk-SK, fy-NL, pt-BR, da-DK, lt-LT, ja-JP-mac, gl-ES, sq-AL, he-IL, en-GB, fa-IR, km-KH, uk-UA, hr-HR, be-BY, ro-RO, es-AR, hu-HU, nb-NO, fi-FI, ca-AD, ar, ml-IN, wa-BE, is-IS, mk-MK
var
//	常用字: 依語系常用度排序。
character_signature = {
	...
	//	<a href="http://ja.wikipedia.org/wiki/CJK%E4%BA%92%E6%8F%9B%E6%BC%A2%E5%AD%97" accessdate="2012/3/17 14:6">CJK互換漢字</a>
	...
	//	加上 한글(朝鲜字母): Hangul Syllables ハングル, Hangul Jamo ハングル字母, Hangul Compatibility Jamo ハングル互換字母
	'ko-KR' : '\uAC00-\uD7A3\u1100-\u11FF\u3131-\u318E',
	// http://hunjang.blogspot.tw/2004/03/korean-language-frequency-of-syllables.html
	// The most frequent syllables are i, ta, nûn, ûl, ka, ko, kû, e, chi, ô (이 다 는 을 가 고 에 지 어) in that order.
	...
	//	基本字母.
	//	<a href="http://en.wikipedia.org/wiki/Letter_frequency" accessdate="2012/3/17 15:55">Letter frequency</a>:
	...
	//	<a href="http://mtgap.bilfo.com/letter_frequency.html" accessdate="2012/3/17 16:1">LETTER FREQUENCY</a>:
	//	Punctuation Frequency: , . - " _ ' ) ( ; = : / * ! ? $ > { } [ ] \ + | & < % @ # ^ ` ~
	'en-US' : ' etaoinshrdlcETAOINSHRDLC,.\\-"0-9',
	//	<a href="http://zh.wikipedia.org/wiki/Unicode%E5%AD%97%E7%AC%A6%E5%88%97%E8%A1%A8#.E5.9F.BA.E6.9C.AC.E6.8B.89.E4.B8.81.E5.AD.97.E6.AF.8D.E5.92.8C.E6.8B.89.E4.B8.81.E7.AC.A6.E8.99.9F" accessdate="2012/3/17 14:7" title="Unicode字符列表">基本 Latin 拉丁字母和拉丁符號</a>
	'x-general' : '\u0020-\u007e'
},
//	次常用字, 罕用字
seldom_signature = {
	...
	//	<a href="http://ja.wikipedia.org/wiki/%E5%8D%8A%E8%A7%92%E3%82%AB%E3%83%8A" accessdate="2012/8/3 23:50">半角カナ</a>・全角形 可能在以 'EUC-JP' 讀取 'EUC-KR' 或 'GB 2312' 時大量出現而造成誤判。
	...
	//	<a href="http://ja.wikipedia.org/wiki/CJK%E4%BA%92%E6%8F%9B%E6%BC%A2%E5%AD%97" accessdate="2012/3/17 14:6">CJK互換漢字</a>
	//	韓国の文字コード規格KS X 1001 (収録当時の規格番号はKS C 5601) に含まれる重複漢字との往復変換を保証するために収録された漢字。
	//	半角・全角形.
	'ko-KR' : '\uF900-\uFA0B\uFFA1-\uFFDC',
...
/**
 * @see <a href="http://ja.wikipedia.org/wiki/Unicode#.E4.B8.80.E8.A6.A7" accessdate="2012/3/17 14:17">Unicode一覧</a>
 * @returns
 */
	...
	if (!signature_RegExp) {
		...
		for (i in character_signature) {
			...
			if ((new language_tag(i)).language && i !== 'en-US') {
				library_namespace.debug('Adding [' + i + ']', 2, 'guess_text_language.get_signature_RegExp');
				guess_text_language.boundary[i] = 0;
			}
		}

		for (i in seldom_signature) {
			signature_RegExp[i + guess_text_language.seldom_postfix]
				= set_RegExp(seldom_signature[i]);
		}
		...
		signature_RegExp['ja-JP' + guess_text_language.seldom_postfix] = new RegExp(char_array.join(''), 'g');
		*/

		if (library_namespace.is_debug(3)) {
			for (char_array in signature_RegExp){
				library_namespace.debug(char_array + ': ' + signature_RegExp[char_array], 3, 'guess_text_language.get_signature_RegExp');
			}
		}
	}
...
function guess_String_language(str) {
 ...
 return ascii+asciiHigh==len?binary_charset
	...
	:kanji>.2*(len-ascii)?	jianhuazi>.1*kanji?kokuji>.02*kanji?'unicode':'GB 2312':kokuji>.02*kanji?'Shift_JIS':'Big5'
	:'';//unicode
};
...
function guess_encoding(FN){
 if(!is_file(FN))return FN.length>64?guess_String_language(FN):guess_encoding.unknown_encoding;
 open_file(FN,binary_charset);
 if(!AdoEnums)return guess_encoding.unknown_encoding;
 ...
 if(code){ADO_Stream.Close();return code;}

 if(!code){
  //	將 Shift_JIS 排在 GB 2312 與 Big5 前面是因為 Shift_JIS 常符合 GB 2312，且 Shift_JIS 之判定相當嚴。
  if(!code)ADO_Stream.Position=0,ADO_Stream.Charset='Shift_JIS',code=guess_String_language(ADO_Stream.ReadText(900),ADO_Stream.Charset);
  //	將 GB 2312 排在 Big5 前面是因為 GB 2312 常用字在 Big5 中常常是0x8000之後的常用字，Big5 常用字卻常常是 GB 2312 中奇怪字碼與罕用字。
  if(!code)ADO_Stream.Position=0,ADO_Stream.Charset='GB 2312',code=guess_String_language(ADO_Stream.ReadText(2000),ADO_Stream.Charset);
  if(!code)ADO_Stream.Position=0,ADO_Stream.Charset='Big5',code=guess_String_language(ADO_Stream.ReadText(2000),ADO_Stream.Charset);
 }
...
//	將 GB 2312 排在Big5前面是因為 GB 2312 常用字在Big5中常常是0x8000之後的常用字，Big5常用字卻常常是 GB 2312 中奇怪字碼與罕用字
