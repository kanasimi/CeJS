...
typeof CeL === 'function' && CeL.run({
	// module name
	name : 'application.net.wiki.template_functions.dewiki',
...
function module_code(library_namespace) {
	...
	function expand_template_Kopfzeile_Synchronisation(options) {
		var parameters = this.parameters;
		return 'id="' + (parameters.id || 'Synchronisation') + '"';
	}
	...
	wiki_API.template_functions.functions_of_site[module_site_name] = {
		...
		// 用於 table。
		'Kopfzeile Synchronisation' : {
			properties : {
				expand : expand_template_Kopfzeile_Synchronisation
			}
		}
	};
