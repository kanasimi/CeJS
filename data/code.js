 ...
 *            {Function}handler: 處理 source text (非 escape sequence) 之 function,<br />
...
format_parser.default_parser = function(object, format, usage) {
	...
	while (matched = search.exec(format)) {
		...
		if (parse_matched)
			...
			// matched = [matched slice (normal + escape sequence), normal, escape character, format pattern, format command];
			// 處理一般字串。
			result.push(normal_parser ? normal_parser(object, matched[1], usage)
					: matched[1]);
			// 處理一般 format。
	...
	result.push(normal_parser ? normal_parser(object, matched, usage) : matched);
...
var backslash_parser = new format_parser('\\', {
		u : {pattern : /[\da-z]{4}/i ,handler : hex_to_Unicode},
		U : {pattern : /[\da-z]{8}/i, handler : hex_to_Unicode},
		x : {pattern : /[\da-z]{2}/i, handler : hex_to_Unicode},
		// '"' : '\"', "'" : "\'", '\\' : '\\',
		b : '\b',
		t : '\t',
		n : '\n',
...
function unnested_formatter_change_status(status, controller, _this,
	...
	// start
	parameter_result = changed_to_value.length ? _this.start(changed_to_value)
			: '';
...
function unnested_formatter_convert_Array(format_Array, _this, meta_status,
	...
	for (; index < length; index++) {
		...
		if (is_controller(item)) {
			if (controller) {
				if (!cloned) {
					controller = cloned = Object.assign(null_Object(), controller);
					//cloned = true;
				}
			...
			formatted_result.push(Array.isArray(item)?unnested_formatter_convert_Array(item, _this,
						status_now):item);
		}
	}
...
//create instence
//<a href="http://docs.aegisub.org/manual/ASS_Tags" accessdate="2012/4/21
//13:16">ASS Tags - Aegisub Manual</a>,
//<a href="http://www20.atwiki.jp/ass_advancedssa/" accessdate="2012/4/21
...
ass_tag.reduce = function(tag) {
	return tag && typeof tag === 'string' ? tag.replace(/}{\\/g, '\\').replace(
			/{[^{}]+\\r([}\\])/g, '{\\r$1').replace(/(\n[^{}\n]*){\\r}/g, '$1')
			.replace(/\s*{\\r}(\s*\r?\n)/g, '$1')
			.replace(/(\\r}[^{]*){\\r}/g, '$1')
			.replace(/(\\r}[^{]*){\\r\\/g, '$1{') : tag;
