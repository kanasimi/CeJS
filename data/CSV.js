 ...
 *      accessdate="2010/1/1 0:53">John Hax: A lesson of RegExp: 50x faster with
 *      just one line patch</a>,<br />http://misoproject.com/dataset/
 * 
...
function parse_CSV(data, config) {
	...
	text_qualifier_replace_from,
	last_index_of_data = 0,
	...
	function add_row(row) {
		...
		if (!handle_row || (use_row = handle_row(row, row_count)) === undefined)
			use_row = row;
			...
			if (isNaN(to_Object)) {
				...
				if (column_index in row) {
					if (row[column_index] in table) {
						library_namespace.warn('parse_CSV: conflict record name: [' + row[column_index] + '], index: ' + column_index);
					}
	...
	if (select_column !== undefined && select_column !== false) {
		library_namespace.debug('select_column: ' + select_column, 2, 'parse_CSV');
	...
	if (!Array.isArray(handle_array)
			&& (!select_column || !library_namespace.is_Object(handle_array)))
		handle_array = undefined;
	library_namespace.debug('handle_array: ' + handle_array, 2, 'parse_CSV');
	...
	field_pattern.push('(?:[', config('field_delimiter'), ']|([', config('line_separator'), ']))?');
	field_pattern = new RegExp(field_pattern.join(''), 'g');
	if(text_qualifier)
		//	e.g., /(["']){2}/g
		text_qualifier_replace_from = new RegExp('([' + text_qualifier + ']){2}', 'g');
	library_namespace.debug('use field pattern: ' + field_pattern, 2, 'parse_CSV');

	// main loop
	while (last_index_of_data < data_length && (matched = field_pattern.exec(data))) {
		last_index_of_data = field_pattern.lastIndex;
		// matched = [fill pattern, extra(error?), field data[, field data], line separator]
		library_namespace.debug(last_index_of_data + '/' + data_length + ' ' + row_count + '<br />' + matched.join('<br />→ '), 4, 'parse_CSV');

		if (library_namespace.is_debug() && matched[1]) {
			// check if data is valid.
			library_namespace.warn('parse_CSV: Can not parse data (無法辨識的資料): [' + matched[1] + ']');
		}
		...
		if (handle_array
				...
				&& typeof (tmp = column_count in handle_array ? handle_array[column_count]
					: title_row_for_Object && ((tmp = title_row_for_Object[column_count]) in handle_array) && handle_array[tmp])
					=== 'function'
				) {
			library_namespace.debug('handle_array(' + value + ')', 2, 'parse_CSV');
		...
		if (title_row_for_Object) {
			...
			library_namespace.debug('[' + (to_Object === undefined||column_count<to_Object?row_count:row[title_row_for_Object[to_Object]]+'(' + row_count+')')+ '].' + title_row_for_Object[column_count] + ' = [' + value + ']', 3, 'parse_CSV');
		} else {
			row.push(value);
			library_namespace.debug('[' + (to_Object === undefined||column_count<to_Object?row_count:row[to_Object]+'('+row_count+')')+ '][' + column_count + '] = [' + value + ']', 3, 'parse_CSV');
		}
		column_count++;
		...
		if (matched[auto_text_qualifier ? 4 : 3]) {
			...
			if (title_passed) {
				...
				if (select_column && !title_row_for_Object) {
					...
					//	cache
					title_row_for_Object_length = title_row_for_Object.length;
					library_namespace.debug('title_row_for_Object_length = ' + title_row_for_Object_length, 3, 'parse_CSV');
				}
			}
	...
	if (library_namespace.is_debug() && last_index_of_data < data_length) {
		library_namespace.warn('parse_CSV: Can not parse data (無法/尚未辨識的資料): ('
 ...
 * 
 * @see <a href="http://tools.ietf.org/html/rfc4180">Common Format and MIME Type
 *      for Comma-Separated Values (CSV) Files</a>
 * 
...
function to_CSV_String(CSV_object, config) {
	...
	//
	field_replace_from = new RegExp('[' + field_delimiter + config('line_separator') + ']', 'g'),
	//
	field_replace_chars = {
		'\t' : '\\t',
		'\n' : '\\n',
	...
	field_replace_to = function($0) {
		...
		return ($0 in field_replace_chars) ? field_replace_chars[$0] : '\\' + $0;
	},
	//
	handle_field = config('no_text_qualifier') && text_qualifier
		? config('no_text_qualifier') === 'auto'
		? function(field, i) {
			library_namespace.debug('add field (auto): [' + field + ']', 3);
			if (field === undefined || field === null)
				return '';

			if(handle_array && (i in handle_array) && typeof handle_array[i] === 'function')
				field = handle_array[i](field);

			if (typeof field !== 'string')
				field = '' + field;
			return text_qualifier_tester.test(field) ?
						text_qualifier
						+ field.replace(text_qualifier_replace_from, text_qualifier_replace_to)
						+ text_qualifier
					: field.replace(field_replace_from, field_replace_to);
		}
		: function(field, i) {
			library_namespace.debug('add field (no text qualifier): [' + field + ']', 3);
			if (field === undefined || field === null)
				return '';
		...
		: function(field, i) {
			library_namespace.debug('add field (add text qualifier): [' + field + ']', 3);
			if (field === undefined || field === null)
	...
	if (typeof select_column === 'number' || typeof select_column === 'string')
	...
	function add_row_by_selectd(row) {
		var i = 0, array = [], index;
		library_namespace.debug('push ' + select_column_length + ' fields', 2);
		if (handle_row)
			row = handle_row(row, CSV.length);
		for (; i < select_column_length; i++) {
			index = select_column[i];
			if (library_namespace.is_debug() && !(index in row))
				library_namespace.warn('unknown index: [' + index + '] @ record #' +  CSV.length);
			array.push(handle_field(row[index], i));
		}
		library_namespace.debug('push selected: [' + array.join(field_delimiter) + ']', 2);
	...
	function add_row_by_data(row) {
		...
		if (Array.isArray(row)) {
			library_namespace.debug('push ' + row.length + ' fields: [' + row +']', 2);
		...
		library_namespace.debug('push: [' + array.join(field_delimiter) + ']', 2);
		CSV.push(array.join(field_delimiter));
	}
	...
	library_namespace.debug('no_text_qualifier: ' + config('no_text_qualifier'), 2);
	library_namespace.debug('text_qualifier_replace_from: ' + text_qualifier_replace_from, 2);
	...
	if (Array.isArray(CSV_object)) {
		...
		library_namespace.warn('to_CSV_String: unknown CSV data type: ' + CSV_object);
		return;
	}
...
to_CSV_String.config = (new library_namespace.setting_pair( {}, default_config,
		{
...
*/
	...
	for (_m = '((|[^' + _f.text_qualifier + _m
		...
		_a = _f.text_qualifier.charAt(_i), _b[_a] = new RegExp(_a + _a, 'g'), _m += '|'
			+ _a + '(([^' + _a
			// +c: for Opera bug
			+ c
			// 不用 [^'+_a+']+| 快很多
...
//old_parse_CSV.title_word='t';	//	data[old_parse_CSV.title_word]=title row array
//old_parse_CSV.fd=';',old_parse_CSV.text_qualifier='"',alert(parse_CSV('"dfdf\nsdff";"sdf""sadf\n""as""dfsdf";sdfsadf;"dfsdfdf""dfsadf";sfshgjk',1).join('\n'));WScript.Quit();
 ...
 * @param has_title the data has a title line
...
/*
 if(!FD||!text_qualifier){
  var a,b,i=0,F='[,;:	\s]',T='[\'"]',r=new RegExp('(^'+(text_qualifier||T)+'|('+(text_qualifier||T)+')('+(FD||F)+')('+(text_qualifier||T)+')|'+(text_qualifier||T)+'$)','g');
  F={},T={};
  try{
  ...
  if(!text_qualifier){a=b=0;for(i in T)if(T[i]>a)a=T[b=i];text_qualifier=b;}
 }
*/
 if(!text_qualifier){
  l=t.indexOf('\n');
...
to_CSV = function(csv_object, config) {
	...
	if (text_qualifier)
		...
		text_qualifier_RegExp = new RegExp('\\' + text_qualifier, 'g'),
		text_qualifier_replace = text_qualifier + text_qualifier;
	else if (line_delimiter === '\n')
		text_qualifier_tester = line_delimiter, text_qualifier_RegExp = /\n/g,
		text_qualifier_replace = '\\n';
...
function quoteCSVfield(t,d){
 ...
 for(var i=0,j,rd=new RegExp(d,'g'),d2=d+d;i<t.length;i++){
  ...
  if(Array.isArray(t[i]))t[i]=t[i].join(',');
 }
 return t.join('\n')+'\n';
}
*/
